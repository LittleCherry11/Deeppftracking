...
def DataAugment(img,gt_box,flip=True):
    ...
    h,w,c=img.shape
    #scale
    s1=np.random.rand(1)*0.2
    s2=np.random.rand(1)*0.2
    print "s1: %f"%s1
    print "s2: %f"%s2
...
def main(args):
    ...
    log_name = 'log_1017_onestage34.txt'
        ...
        for sequence in datafull_seq:  # datafull_seq
            ...
            for id in np.arange(0, nFrame):
                ...
                if init_id == False:
                    ...
                    #pca_f, scaler_f = featmap_pca2(features0,ncompnents=128)
            ...
            res_f = open('results_onestage34/%s.txt'%sequence,'w')
            pred_hist[:,2:] = pred_hist[:,2:] - pred_hist[:,:2]
            res_f = write_res(pred_hist,res_f)
            res_f.close()
    log_file.close()
    pkl = open('results_1017_34.pkl', 'w')
